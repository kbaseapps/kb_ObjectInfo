...
class CreateAssemblyReport:
    ...
    def assembly_metadata(self,obj_list):
        ...
        for assembly_metadata in obj_list['data']:
            name = "Assembly Data Object"
            if 'info' in assembly_metadata:
                name = assembly_metadata['info'][1]

            # Create the row for the one assembly
            assem_list = [name]
            for item in list:
                if item in assembly_metadata['data']:
                    assem_list.append(str(assembly_metadata['data'][item]))
                else:
                    assem_list.append(" ")

            if 'fasta_handle_info' in assembly_metadata['data'] and 'node_file_name' in assembly_metadata['data']['fasta_handle_info']:
                assem_list.append(assembly_metadata['data']['fasta_handle_info']['node_file_name'])
            else:
                assem_list.append(" ")
    ...
    def assembly_dnabases(self,obj_list):
        ...
        for assembly_metadata in obj_list['data']:
            ...
            if 'info' in assembly_metadata:
                name = assembly_metadata['info'][1]
            if 'dna_size' in assembly_metadata['data']:
                dna_size = assembly_metadata['data']['dna_size']
            
            assem_list = [name,str(dna_size)]
            for base in ["A","C","G","T"]:
                pct = 100 * assembly_metadata['data']['base_counts'][base] / dna_size
                assem_list.append(str(assembly_metadata['data']['base_counts'][base]))
    ...
    def assembly_contigs(self,obj_list):
        ...
        for assembly_metadata in obj_list['data']:
            ...
            if 'contigs' in assembly_metadata['data']:
                myContig = assembly_metadata['data']['contigs']
                for ctg in myContig:
    ...
    def assembly_dna(self,obj_list):
        ...
        for assembly in obj_list['data']:
            ...
            if 'info' in assembly_metadata:
                ...
                report_path = os.path.join(self.scratch, name + '.fna')
